          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 6 }}>
            {allStats.map((s, i) => (
              <div key={i} style={{ background: 'var(--bg2)', border: `1px solid ${s.color}30`, borderRadius: 8, padding: '8px' }}>
                <div style={{ fontSize: 14, fontWeight: 700, color: s.color, textShadow: `0 0 10px ${s.color}80` }}>
        ...
        {gremlins.map(g => {
          ...
          return (
            <div key={g.id} className="card"
              ...
              onClick={() => onSelect(g)}>
              <div style={{ width: 40, height: 40, borderRadius: 8, background: 'var(--bg3)', border: `1px solid ${color}50`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, flexShrink: 0 }}>
                {ROLE_ICONS[g.role] || '👾'}
              </div>
